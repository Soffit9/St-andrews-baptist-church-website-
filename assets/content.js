...
const CMS_SCHEMA = {
  ...
  sermons: {
    ...
    fields: [
      ...
      { id: "youtube_id", label: "Video Link (paste a YouTube or Facebook video link)", type: "text", default: "" }
    ],
    images: []
  },
  events: {
...
/* Turns a pasted video link (YouTube OR Facebook) into the right embed
   iframe. The church posts sermons to both — Facebook historically,
   YouTube going forward — so this needs to handle whichever one was
   pasted rather than assuming YouTube like the old version did.
   Facebook's embed plugin is considerably less reliable than YouTube's —
   it often can't resolve "share/v/" style short-links (common when
   sharing from the mobile app), and can fail silently depending on the
   video's privacy setting. Rather than risk a dead end, Facebook links
   always get a "Watch on Facebook" fallback link alongside the embed
   attempt, so there's always a working way to actually see the video. */
function videoEmbedHtml(url, heightPx) {
  if (!url) return "";
  const str = String(url).trim();
  if (/facebook\.com/i.test(str)) {
    const encoded = encodeURIComponent(str);
    const h = heightPx || 315;
    return `<div>
      <iframe src="https://www.facebook.com/plugins/video.php?href=${encoded}&show_text=false" width="100%" height="${h}" style="border:0;overflow:hidden;max-width:100%;display:block" scrolling="no" frameborder="0" allowfullscreen allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share"></iframe>
      <p style="margin-top:8px"><a class="text-link" target="_blank" rel="noopener" href="${str}">▶ Watch on Facebook →</a> <span style="font-size:12px;color:var(--muted)">(use this if the box above says "unavailable")</span></p>
    </div>`;
  }
...
function cmsApplyToPage(pageKey) {
  ...
  // Sermons: swap in a video embed if one's been set (YouTube or Facebook)
  if (pageKey === "sermons" && content.youtube_id) {
    const slot = document.querySelector("#sermon-video");
    if (slot) {
      const isFacebook = /facebook\.com/i.test(content.youtube_id);
      slot.innerHTML = videoEmbedHtml(content.youtube_id);
      if (isFacebook) {
        // Facebook's embed isn't absolutely-positioned like YouTube's, so
        // the usual "fill the padded box" trick collapses it to zero
        // height instead of showing anything — let it size normally instead.
  ...
  if (homeVideoSlot) {
    ...
    if (sermonContent.youtube_id) {
      const isFacebook = /facebook\.com/i.test(sermonContent.youtube_id);
      homeVideoSlot.innerHTML = videoEmbedHtml(sermonContent.youtube_id);
      if (isFacebook) {
        homeVideoSlot.style.position = "static";
        homeVideoSlot.style.paddingBottom = "0";
        homeVideoSlot.style.height = "auto";
      } else {

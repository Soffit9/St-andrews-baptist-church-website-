...
const CMS_SCHEMA = {
  global: {
    ...
    fields: [],
    images: [
      { id: "logo", label: "Church Logo", hint: "Shows in the header on every page.", sizable: true, positionable: true, sizeMin: 32, sizeMax: 90, sizeDefault: 44 }
    ]
  },
  home: {
    label: "Homepage",
...
function cmsStorageWorks() {
  try {
    localStorage.setItem("sabc_test", "1");
    const ok = localStorage.getItem("sabc_test") === "1";
    localStorage.removeItem("sabc_test");
    return ok;
  } catch { return false; }
}
...
function cmsSavePage(pageKey, data) {
  ...
  try {
    ...
    console.error("cmsSavePage failed to save:", err);
...
function cmsRevertLogEntry(entry) {
  const revertData = {};
  let any = false;
  ...
  return cmsSavePage(entry.page, revertData);
}
...
function cmsRevertRawEntry(entry) {
  const c = (entry.changes || [])[0];
  if (!c || !c.revertable || !c.isRaw) return { ok: false };
  try {
    if (c.revertPath) {
      const data = JSON.parse(localStorage.getItem(c.revertKey) || "{}");
      data[c.revertPath] = c.rawOld;
      localStorage.setItem(c.revertKey, JSON.stringify(data));
    } else {
      localStorage.setItem(c.revertKey, JSON.stringify(c.rawOld));
    }
    return { ok: true };
  } catch (err) {
    return { ok: false, error: err };
...
function cmsLogChange(page, changes) {
  ...
  const log = JSON.parse(localStorage.getItem(CMS_LOG_KEY) || "[]");
  log.unshift({ ts: new Date().toISOString(), page, changes });
  if (log.length > 500) log.length = 500;
  safeSetItem(CMS_LOG_KEY, JSON.stringify(log));
}
...
function cmsApplyToPage(pageKey) {
  ...
  if (global.logo) {
    ...
    const logoW = Math.round(logoH * (58 / 44)); // keep the original box's proportions
    const logoPos = global.logo_pos ?? 50;
    document.querySelectorAll("[data-image='logo']").forEach(el => {
      el.style.height = logoH + "px";
      el.style.width = logoW + "px";
      el.innerHTML = `<img src="${global.logo}" alt="Church logo" style="width:100%;height:100%;object-fit:contain;object-position:50% ${logoPos}%;border-radius:7px">`;
    });
  }

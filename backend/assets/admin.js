  ...
  if (step1 && step2) {
    ...
    step1.addEventListener("submit", async e => {
      ...
      try {
        ...
        const totpNote = data.totpAvailable ? " Or, if you've set up an authenticator app, just type the current code from that instead." : "";
        if (data.emailed) {
          if (codeBox) codeBox.innerHTML = `A code was just emailed to <b>${email}</b>.${totpNote}`;
        } else if (codeBox) {
          codeBox.innerHTML = data.totpAvailable
            ? `Enter the current code from your authenticator app.`
            : `Email isn't set up yet, so here's the code directly (this box goes away once real email sending is turned on): <strong id="demo-code-display">${data.demo_code}</strong>`;
        }

...
- **Who's Who, Gallery, Events, the Calendar, and Sermon Archive still save to whichever browser you're using** — not shared across devices yet. Prayer Requests are now the first thing moved over to real shared storage — the rest follow the same pattern next.
- **Gallery photos** only appear on the device that uploaded them, for the same reason. Best used today by uploading straight from the computer you'll present from.

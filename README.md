...
- **Prayer form no longer complains about invalid email format** — the field now accepts anything, since it's optional anyway.
- **Fixed the "two codes, only one worked" login bug** — the button now locks while a code is being sent, so it can't accidentally fire twice.
- **Settings is a real page now** — change the shared password, or set up/replace/turn off the authenticator app, right from the browser. No more needing SSH access just to scan a QR code.
- **Homepage Photos slider finally has a visible label** — it only had an invisible hover tooltip before, genuinely confusing.
- **Events now roll off the public page automatically** — the homepage and Events page only show through the end of next month; nothing is ever deleted from the admin's actual data, this only limits what's *displayed*.
- **Gallery: added the same full-screen click-to-view to the admin manager**, not just the public download page.
